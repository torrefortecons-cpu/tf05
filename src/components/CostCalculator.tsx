...
export default function CostCalculator() {
  ...
  return (
    <div className="space-y-1 sm:space-y-3 text-slate-800">
      
      {/* Title Header Card */}
      <div className="bg-white p-2.5 sm:p-4 py-1.5 sm:py-2.5 rounded-xl sm:rounded-2xl border border-slate-100 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-1 sm:gap-3">
        <div>
          <h3 className="text-xs sm:text-base md:text-lg font-extrabold text-[#051125] flex items-center gap-1.5 sm:gap-2">
            <Coins className="w-4.5 h-4.5 sm:w-5.5 sm:h-5.5 text-[#C67C3E] shrink-0" />
            Sistema Integrado de Costeo y Fijación de Precios de Producción
          </h3>
          <p className="text-[10px] sm:text-[12.5px] font-semibold text-slate-950 mt-0.5 leading-snug">
        ...
        <div className="flex gap-1 sm:gap-1.5 shrink-0 justify-between sm:justify-start w-full sm:w-auto mt-1 sm:mt-0">
          <div className="px-2 py-0.5 sm:px-3 sm:py-1 bg-[#051125]/5 rounded-lg sm:rounded-xl border border-[#051125]/10 text-center flex-1 sm:flex-initial">
            <div className="text-[8px] sm:text-[9px] text-slate-500 font-bold uppercase tracking-wider">Materiales Reg.</div>
            <div className="text-xs sm:text-sm font-extrabold text-[#051125]">{materiales.length}</div>
          </div>
          <div className="px-2 py-0.5 sm:px-3 sm:py-1 bg-emerald-50 rounded-lg sm:rounded-xl border border-emerald-100 text-center flex-1 sm:flex-initial">
            <div className="text-[8px] sm:text-[9px] text-emerald-600 font-bold uppercase tracking-wider font-sans">Soportes Costeados</div>
            <div className="text-xs sm:text-sm font-extrabold text-[#051125]">{uniqueProducts.length}</div>
          </div>
        </div>
      </div>

      {/* Sub Tabs switcher */}
      <div className="grid grid-cols-3 sm:flex border border-slate-200 bg-white p-0.5 sm:p-1 rounded-xl gap-0.5 sm:gap-1.5 w-full sm:w-fit mt-0">
        <button
          onClick={() => setActiveSubTab('materiales')}
          className={`flex flex-col sm:flex-row items-center justify-center gap-0.5 sm:gap-1.5 px-0.5 py-1 sm:px-3 sm:py-1.5 text-[8px] sm:text-xs font-bold rounded-lg cursor-pointer transition-all min-w-0 text-center ${
            activeSubTab === 'materiales'
              ? 'bg-[#051125] text-[#C67C3E] border border-[#C67C3E]/20 shadow-sm'
      ...
      {/* TAB 1: MATERIA PRIMA */}
      {activeSubTab === 'materiales' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-1.5 sm:gap-3.5 animate-fade-in">
          
          {/* Add form */}
          <div className="lg:col-span-4 bg-white p-3 sm:p-5 rounded-xl sm:rounded-2xl border border-slate-200 shadow-sm space-y-1.5 sm:space-y-2.5 h-fit">
            <h4 className="font-bold text-sm text-[#051125] pb-1 sm:pb-1.5 border-b flex items-center gap-1.5">
            ...
            <form onSubmit={handleAgregarMaterial} className="space-y-1.5 sm:space-y-3 mt-1">
              <div>
                <label className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider mb-0.5 sm:mb-1 font-sans">Nombre del Material:</label>
                <input 
                  type="text" 
                  value={matNombre}
                  onChange={(e) => setMatNombre(e.target.value)}
                  placeholder="Ej: Tubo de Acero 40x40x1.2 o Pernos 1/2"
                  className="w-full text-sm px-4 py-2 sm:py-2.5 bg-white border-2 border-slate-400 hover:border-slate-500 rounded-lg text-slate-800 font-semibold focus:ring-4 focus:ring-[#C67C3E]/20 focus:border-[#C67C3E] focus:outline-none transition-all placeholder:text-slate-400 shadow-sm"
                />
              </div>

              <div className="grid grid-cols-2 gap-2 sm:gap-3.5">
                <div>
                  <label className="block text-[11.5px] font-bold text-slate-700 uppercase tracking-wider mb-0.5 sm:mb-2 font-sans">Tipo de Material:</label>
                  <select
                    value={matTipo}
                    onChange={(e) => setMatTipo(e.target.value)}
                    className="w-full text-sm px-3 py-2 sm:py-2.5 bg-white border-2 border-slate-400 hover:border-slate-500 rounded-lg text-slate-800 font-semibold focus:outline-none focus:border-[#C67C3E] focus:ring-4 focus:ring-[#C67C3E]/20 cursor-pointer shadow-sm"
                  >
                ...
                <div>
                  <label className="block text-[11.5px] font-bold text-slate-700 uppercase tracking-wider mb-0.5 sm:mb-2 font-sans">Unidad de Medida:</label>
                  <select
                    value={matUnidad}
              ...
              <div className="grid grid-cols-2 gap-2 sm:gap-3.5">
                <div>
                  <label className="block text-[11.5px] font-bold text-slate-700 uppercase tracking-wider mb-0.5 sm:mb-2 font-sans">Cantidad Comprada:</label>
                  <input 
                    type="text" 
                    inputMode="decimal"
                    value={matCantidad}
                    ...
                    placeholder="Ej: 60"
                    className="w-full text-sm px-4 py-2 sm:py-2.5 bg-white border-2 border-slate-400 hover:border-slate-500 rounded-lg text-slate-800 font-semibold focus:ring-4 focus:ring-[#C67C3E]/20 focus:border-[#C67C3E] focus:outline-none shadow-sm"
                  />
                </div>

                <div>
                  <label className="block text-[11.5px] font-extrabold text-[#C67C3E] uppercase tracking-wider mb-0.5 sm:mb-2 font-sans">Monto Compra (Bs):</label>
                  <input 
                    type="text" 
                    inputMode="decimal"
                    value={matPrecio}
                    ...
                    placeholder="Ej: 320"
                    className="w-full text-sm px-4 py-2 sm:py-2.5 bg-amber-50 border-2 border-amber-400 hover:border-amber-500 text-slate-900 font-black focus:ring-4 focus:ring-[#C67C3E]/20 focus:border-[#C67C3E] focus:outline-none rounded-lg shadow-sm"
                  />
                </div>
              </div>

              <div>
                <label className="block text-[11.5px] font-bold text-slate-700 uppercase tracking-wider mb-0.5 sm:mb-2 font-sans">Proveedor de Adquisición:</label>
                <input 
                  type="text" 
                  value={matProveedor}
              ...
              <div>
                <label className="block text-[11.5px] font-bold text-slate-700 uppercase tracking-wider mb-0.5 sm:mb-2 font-sans">Fecha de Compra:</label>
                <input 
                  type="date" 
                  value={matFecha}
              ...
              <button
                type="submit"
                className="w-full bg-[#051125] text-white hover:bg-[#C67C3E] py-2 sm:py-3.5 px-4 rounded-xl text-sm font-black transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-md mt-2.5 sm:mt-4 hover:shadow-lg active:scale-[0.98]"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-1.5 sm:gap-3.5 animate-fade-in">
          ...
          <div className="lg:col-span-4 bg-white p-3 sm:p-5 rounded-xl sm:rounded-2xl border border-slate-200 shadow-sm space-y-1.5 sm:space-y-2.5 h-fit">
            ...
            {materiales.length === 0 ? (
              ...
              <form onSubmit={handleAgregarLineaProducto} className="space-y-1.5 sm:space-y-3 mt-1">
                <div>
                  <label className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider mb-0.5 sm:mb-1 font-sans">Nombre del Producto / Soporte:</label>
                  <select 
                    value={prodNombre}
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-0.5 sm:mb-1 font-mono">Insumo del Inventario:</label>
                  <select
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-0.5 sm:mb-1 font-mono">Cantidad a Usar por Unidad de Soporte:</label>
                ...
                {selectedMaterialObject && parseFloat(prodUso) > 0 && (
                  <div className="p-2 bg-slate-50/50 border border-slate-200/60 rounded-xl space-y-0.5 text-[11px] font-sans text-slate-600">
                ...
                <button
                  type="submit"
                  className="w-full bg-[#051125] text-white hover:bg-[#C67C3E] py-2 sm:py-2.5 px-4 rounded-xl text-xs font-black transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-md mt-1.5 sm:mt-2"
                >
                  <Plus className="w-4 h-4" />
      ...
      {activeSubTab === 'costos' && (
        <div className="space-y-1.5 sm:space-y-3.5 animate-fade-in" id="costs-form-anchor">
          
          {/* Top Form to Edit / input Op costs */}
          <div className="bg-white p-2.5 sm:p-3.5 px-3 sm:px-4 rounded-xl border border-slate-200 shadow-sm space-y-1 sm:space-y-2">
